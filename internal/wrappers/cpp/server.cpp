...
namespace kuzzleio {
  ...
  std::string Server::getLastStats(time_t start, time_t end, query_options* options) Kuz_Throw_KuzzleException {
    string_result* r = kuzzle_get_last_stats(_server, start, end, options);
    if (r->error != NULL)
        throwExceptionFromStatus(r);
    std::string ret = r->result;
    delete(r);
